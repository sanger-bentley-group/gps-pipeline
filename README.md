...
- These tools are used in `SAM_TO_SORTED_BAM`, `REF_COVERAGE` and `SNP_CALL` processes in `mapping.nf` module
...
- This tool is used in `GET_REF_GENOME_BWA_DB_PREFIX` and `MAPPING` processes in `mapping.nf` module
...
- This Docker image provides containerised environment for processes within multiple modules 
...
- This Docker image provides containerised environment for `HET_SNP_COUNT` process in `mapping.nf` module 

[fastp](https://github.com/OpenGene/fastp)
- Shifu Chen, Yanqing Zhou, Yaru Chen, Jia Gu; fastp: an ultra-fast all-in-one FASTQ preprocessor, Bioinformatics, Volume 34, Issue 17, 1 September 2018, Pages i884–i890, https://doi.org/10.1093/bioinformatics/bty560
- License (MIT): https://github.com/OpenGene/fastp/blob/master/LICENSE
- This tool is used in `PREPROCESS` process in `preprocess.nf` module
...
- Code adapted into `LINEAGE` process in the `lineage.nf` module
...
- This tool is used in `TAXONOMY` process in `taxonomy.nf` module
...
- Code rewritten into `HET_SNP_COUNT` process in the `mapping.nf` module
...
- This tool is used in `MLST` process in `mlst.nf` module
...
- This project is a Nextflow pipeline; Nextflow executable `nextflow` included in this repository
...
- This tool is used in `LINEAGE` process in `lineage.nf` module
...
- This tool is used in `ASSEMBLY_ASSESS` process in `assembly.nf` module
...
- This tool is used in `CREATE_SEROBA_DB` and `SEROTYPE` processes in `serotype.nf` module

[Shovill](https://github.com/tseemann/shovill)
- Torsten Seemann ([@tseemann](https://github.com/tseemann))
- License (GPL-3.0): https://github.com/tseemann/shovill/blob/master/LICENSE
- This tool is used in `ASSEMBLY_SHOVILL` process in `assembly.nf` module
...
- This project uses a Docker image built from a [custom fork](https://github.com/HarryHung/spn-resistance-pbp)
  - The fork changes the Docker image from a Docker executable image to a Docker enviornment for Nextflow integration
  - The Docker image provides containerised environment for `PBP_RESISTANCE` process in `amr.nf` module 
...
- This tool is used in `ASSEMBLY_UNICYCLER` process in `assembly.nf` module